#!/usr/bin/env python3
"""
Standalone scheduled-loop monitoring pipeline for khabarfoori.com.

This file contains the full pipeline and runs continuously on the built-in interval.
"""
...
## Mission
Classify only whether the article is relevant to:
...
If the article is country-specific but has no meaningful relevance to Iran or broad world-level macro/security conditions, do not promote it into a tracked category.
...
## Decision logic
1. Judge the article by its main meaning, not by isolated keywords.
2. Keywords are evidence, not the decision by themselves.
3. Weak overlap is not enough. If the article only mentions words like gold, sanctions, war, or attack in passing, classify `other`.
4. If the event is clearly security-related and directly important for Iran or broad regional/global stability, use `security`.
5. If the event is clearly economic and directly important for Iran or broad global markets, use `economics`.
6. If both security and economics are central and tightly linked in the article, use `security/economics`.
7. When in doubt, be conservative and prefer `other`.
...
def build_news_workbook(rows: List[Dict[str, Any]], target_path: Path):
    ...
    for record_index, row_data in enumerate(rows, start=1):
        ...
        is_notified = row_data.get("وضعیت اطلاع رسانی") == WORKBOOK_NOTIFICATION_TRUE
...
def classification_memory_prompt_text(database: Dict[str, Any]) -> str:
    ...
    entries = classification_memory_entries(database)[:CLASSIFICATION_MEMORY_PROMPT_LIMIT]
    if not entries:
        return "نمونه قبلی برای حافظه طبقه بندی موجود نیست."

    lines = [
        "## Dynamic Classification Memory",
        "این حافظه فقط برای ثبات مرزبندی است و ممکن است خطا داشته باشد.",
        "اگر با متن خبر فعلی سازگار نبود، آن را نادیده بگیر و مستقل تصمیم بگیر.",
    ]
    ...
    prompt_memory_text = prompt_memory_examples_text()
    if prompt_memory_text:
        candidate = "\n\n".join(["\n".join(lines), prompt_memory_text])
        if len(candidate) <= CLASSIFICATION_MEMORY_PROMPT_CHAR_LIMIT:
            return candidate
    return "\n".join(lines)
...
def evaluation_memory_prompt_text(database: Dict[str, Any]) -> str:
    entries = classification_memory_entries(database)[:16]
    lines = [
        "## Evaluation Memory",
        "این حافظه فقط برای کالیبره کردن ارزیابی است و نباید جای خبر فعلی را بگیرد.",
        "اگر خبر جدید با الگوهای قبلی متفاوت بود، حتما بر اساس متن فعلی تصمیم بگیر.",
    ]
    for entry in entries:
        line = (
            f"- {entry['category']} | منطق: {entry['logic'] or 'ندارد'} | "
            f"تیتر: {entry['title'] or 'بدون تیتر'}"
        )
        candidate = "\n".join(lines + [line])
...
class NewsPipeline:
    ...
    def call_llm_text(self, prompt: str, stage: str, costs: Dict[str, Any], logger: CycleLogger) -> str:
        ...
        if stage == "compression" and body["choices"][0].get("finish_reason") == "length":
            logger.event("llm", "compression_truncated_by_api", preview=text[:200])
    ...
    def classification_prompt(self, article: Dict[str, Any], evidence: Dict[str, Any], memory_text: str) -> str:
        ...
        return f"""
شما مسئول طبقه بندی موضوعی خبر پیش از مرحله ارزیابی هستید.
وظیفه شما فقط تعیین دسته خبر است و نباید هنوز شدت اثر، جهت طلا یا وضعیت اطلاع رسانی را تعیین کنید.
...
- فقط یکی از این چهار دسته مجاز است: security ، economics ، security/economics ، other
- هم از شواهد کلیدواژه ای استفاده کن و هم از معنای واقعی خبر.
- اگر خبر صرفا اجتماعی، فرهنگی، محلی، ورزشی، سرگرمی، هنری یا حاشیه ای است، دسته `other` بده.
- اگر خبر علمی، پزشکی، فناوری عمومی، آموزش، فال، چیستان، سبک زندگی، سلبریتی، یا سرگرمی است، به طور پیش فرض `other` است مگر اینکه متن واقعا درباره امنیت یا اقتصاد باشد.
- اگر واژه ای مثل طلا یا تحریم فقط به صورت ضعیف یا حاشیه ای آمده اما موضوع اصلی اقتصادی نیست، اقتصاد نده.
- اگر واژه ای مثل حمله یا جنگ فقط در نقل قول یا حاشیه آمده اما موضوع اصلی امنیتی نیست، امنیت نده.
- اگر مجموع شواهد کلیدواژه ای خیلی کم است و موضوع اصلی خبر مستقیما امنیتی یا اقتصادی نیست، `other` بده.
- matched keyword ها را فقط از بین کلیدواژه های واقعا مشاهده شده در متن برگردان.
- memory_keywords را حداکثر با 3 مورد کوتاه برگردان.
- memory_logic باید یک عبارت کوتاه و موجز باشد، نه توضیح بلند.
- حافظه قبلی ممکن است ناقص یا اشتباه باشد؛ اگر با خبر فعلی ناسازگار بود، حافظه را نادیده بگیر.
- تازگی خبر مهم است: موضوعات جدید را مستقل از سابقه، منصفانه طبقه بندی کن.

حافظه پویای طبقه بندی:
{memory_text}
...
""".strip()
    ...
    def evaluation_prompt(self, article: Dict[str, Any], memory_text: str) -> str:
        return f"""
شما باید این خبر را همزمان از سه نقش ارزیابی کنید:
1. راستی آزمای معتبر بودن و میزان اتکاپذیری خبر
2. تحلیلگر ایرانی اثر خبر بر قیمت طلا و نرخ ارز داخلی
3. تحلیلگر ارتباط خبر با امنیت و ریسک ژئوپلیتیک ایران

فقط بر مبنای همین خبر تصمیم بگیرید. هیچ خبر یا نمونه قبلی را در نظر نگیرید.
از حدس، بزرگنمایی، تفسیر آزاد و امتیازدهی سخاوتمندانه خودداری کنید.
از حافظه فقط برای کالیبره شدن استفاده کن، نه تقلید. اگر با خبر فعلی ناسازگار بود نادیده بگیر.

حافظه کمکی ارزیابی:
{memory_text}

هدف:
برای این خبر فقط 4 خروجی بدهید:
1. اطمینان از وقوع خبر
2. چقدر بر تغییر قیمت طلا اثر دارد؟
3. چقدر با امنیت مرتبط است؟
4. جهت طلا

قواعد سخت:
- فقط JSON معتبر برگردان.
- فقط از متن همین خبر، منبع خبر، و دسته بندی فعلی خبر استفاده کن.
- اگر خبر محلی، اجتماعی، سرگرمی، یا کم اهمیت است، نمره بالا نده.
- برای "زیاد" و "خیلی زیاد" باید متن خبر به صورت روشن و مستقیم به رخداد مهم، نسبتاً معتبر و اثرگذار اشاره کند.
- اگر خبر بیشتر شبیه تحلیل شخصی، لفاظی سیاسی، بازنشر ادعا، شایعه، یا گمانه زنی است، اطمینان را محافظه کارانه نگه دار.
- اگر اثر خبر بر طلا غیرمستقیم، ضعیف، یا چند مرحله ای است، از "متوسط" بالاتر نده.
- اگر ارتباط امنیتی یا اقتصادی مبهم، فرعی، یا حاشیه ای است، از "متوسط" بالاتر نده.
- اگر جهت اثر از متن به طور معقول قابل استنباط نیست، حتماً `?` بده.
- هیچ توضیحی بیرون از JSON ننویس.

تعاریف:
- اطمینان از وقوع خبر:
  خیلی کم = شایعه یا خبر بسیار نامطمئن
  کم = پشتوانه ضعیف یا نقل قول مبهم
  متوسط = محتمل ولی نه کاملا قطعی
  زیاد = خبر روشن و قابل اتکا
  خیلی زیاد = خبر تاییدشده و قطعی

- اثر بر تغییر قیمت طلا:
  خیلی کم = تقریبا بی اثر
  کم = اثر محدود
  متوسط = اثر قابل توجه اما نه شدید
  زیاد = اثر مهم و محسوس
  خیلی زیاد = اثر شدید و مستقیم

- ارتباط با امنیت:
  خیلی کم = تقریبا بی ارتباط
  کم = ارتباط محدود
  متوسط = ارتباط قابل توجه
  زیاد = ارتباط مستقیم و مهم
  خیلی زیاد = ارتباط بسیار مستقیم و شدید
...
منطق تخصصی برای اطمینان از وقوع خبر:
- منبع، لحن خبر، و میزان قطعیت ادعا را بررسی کن.
- اگر خبر به صورت "ادعا شد"، "گفته می شود"، "احتمال"، "منابع آگاه" یا بدون شاهد روشن بیان شده، اطمینان را محدود نگه دار.
- اگر خبر حاوی جزئیات مشخص، نهاد یا مقام روشن، یا بیان قطعی درباره یک رخداد است، می تواند اطمینان بالاتری بگیرد.

منطق تخصصی برای اثر بر طلا از نگاه ایران:
- فقط به اونس جهانی طلا فکر نکن؛ اثر بر نرخ دلار و ارزش ریال در ایران را هم لحاظ کن.
- اگر خبر باعث افزایش ریسک ژئوپلیتیک، فشار تحریمی، اختلال در نفت، فشار ارزی، تورم انتظاری، یا تضعیف ریال شود، این می تواند قیمت طلای داخلی را بالا ببرد.
- اگر خبر باعث کاهش تنش، کاهش ریسک، ثبات ارزی، یا بهبود انتظارات اقتصادی شود، می تواند اثر کاهشی یا خنثی بر طلای داخلی داشته باشد.
- اگر خبر فقط به طور ضعیف یا غیرمستقیم روی بازارها اثر دارد، امتیاز را محافظه کارانه بده.

منطق تخصصی برای ارتباط امنیتی:
- فقط وقتی نمره بالا بده که متن مستقیماً به جنگ، درگیری، حمله، تهدید، امنیت ملی، ریسک ژئوپلیتیک، یا پیامدهای امنیتی روشن مربوط باشد.
- اگر خبر بیشتر دیپلماتیک، رسانه ای، یا سیاسی است و اثر امنیتی آن غیرمستقیم است، نمره را محدود نگه دار.
...
""".strip()
    ...
    def evaluate_article(
        ...
        memory_text = evaluation_memory_prompt_text(database)
