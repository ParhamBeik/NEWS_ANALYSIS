...
backup shell tests and both Docker image builds. `main` deploys after successful CI.
...
staff-level API reads, and fresh local and off-host backups on the VPS. A failed check
...
exists to prevent. `/ops` shows the age of the newest verified dump and its off-host copy;
anything over 36 hours turns red. The copy indicator is unverified if the newest dump has
not been copied, even if an older copy completed recently.
