...
def test_every_page_renders(client):
    for route in ("/", "/review", "/kpi", "/partials/runs"):
...
def test_kpi_shows_an_empty_state_rather_than_fabricated_numbers(client):
    assert "هنوز برچسب تأییدشده‌ای ثبت نشده است" in client.get("/kpi").text
...
def test_submitting_a_label_stores_it_and_advances_the_queue(client):
    ...
    assert "همه موارد صف بازبینی بررسی شده‌اند" in client.get("/review").text
...
def test_an_approved_label_reaches_the_kpi_page(client):
    ...
    assert "هنوز برچسب تأییدشده‌ای ثبت نشده است" not in page
    assert "ماتریس درهم‌ریختگی" in page
